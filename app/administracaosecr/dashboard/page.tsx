import { obterEstatisticasUsuarios } from '@/lib/admin-auth'
import { Users, CreditCard, UserCheck, TrendingUp } from 'lucide-react'

export default async function AdminDashboardPage() {
  const stats = await obterEstatisticasUsuarios()

  const cards = [
    {
      icon: Users,
      label: 'Total de Usuários',
      value: stats.data?.total_usuarios || 0,
      color: 'text-blue-400',
      bgColor: 'bg-blue-900/20',
    },
    {
      icon: CreditCard,
      label: 'Usuários Assinantes',
      value: stats.data?.usuarios_assinantes || 0,
      color: 'text-green-400',
      bgColor: 'bg-green-900/20',
    },
    {
      icon: UserCheck,
      label: 'Usuários Teste',
      value: stats.data?.usuarios_teste || 0,
      color: 'text-orange-400',
      bgColor: 'bg-orange-900/20',
    },
    {
      icon: TrendingUp,
      label: 'Taxa de Conversão',
      value: stats.data?.total_usuarios 
        ? ((stats.data.usuarios_assinantes / stats.data.total_usuarios) * 100).toFixed(1) + '%'
        : '0%',
      color: 'text-purple-400',
      bgColor: 'bg-purple-900/20',
    },
  ]

  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-display font-bold text-brand-clean mb-2">
          Dashboard Administrativo
        </h1>
        <p className="text-brand-clean/70">
          Visão geral da plataforma
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {cards.map((card, index) => {
          const Icon = card.icon
          return (
            <div
              key={index}
              className="bg-brand-royal rounded-2xl p-6 shadow-lg border border-white/10"
            >
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 rounded-xl ${card.bgColor}`}>
                  <Icon size={24} className={card.color} strokeWidth={2} />
                </div>
              </div>
              <p className="text-sm text-brand-clean/70 mb-1 font-medium">
                {card.label}
              </p>
              <p className={`text-3xl font-display ${card.color}`}>
                {card.value}
              </p>
            </div>
          )
        })}
      </div>

      <div className="bg-brand-royal rounded-2xl p-6 shadow-lg border border-white/10">
        <h2 className="text-xl font-display font-bold text-brand-clean mb-4">
          Resumo
        </h2>
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-brand-midnight/50 rounded-xl">
            <span className="text-brand-clean/80">Total de usuários cadastrados</span>
            <span className="text-2xl font-bold text-brand-clean">
              {stats.data?.total_usuarios || 0}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-brand-midnight/50 rounded-xl">
            <span className="text-brand-clean/80">Usuários com plano ativo</span>
            <span className="text-2xl font-bold text-green-400">
              {stats.data?.usuarios_assinantes || 0}
            </span>
          </div>
          <div className="flex items-center justify-between p-4 bg-brand-midnight/50 rounded-xl">
            <span className="text-brand-clean/80">Usuários em teste</span>
            <span className="text-2xl font-bold text-orange-400">
              {stats.data?.usuarios_teste || 0}
            </span>
          </div>
        </div>
      </div>
    </div>
  )
}





